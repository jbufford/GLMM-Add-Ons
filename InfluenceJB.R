          ...
          count = FALSE, delete = TRUE, ...) 
{
  ...
  original.no.estex <- which(substr(names(lme4::fixef(model)), 1, 
                                    6) != "estex.")
  ...
  if (!obs) {
    if (is.null(select)) {
      alt.fixed <- matrix(ncol = n.pred, nrow = n.groups, 
                          data = NA)
      dimnames(alt.fixed) <- list(grouping.names, names(lme4::fixef(model))[original.no.estex])
      alt.se <- matrix(ncol = n.pred, nrow = n.groups, 
                       data = NA)
      dimnames(alt.se) <- list(grouping.names, names(lme4::fixef(model))[original.no.estex])
      alt.vcov <- list()
      alt.test <- matrix(ncol = n.pred, nrow = n.groups, 
                         data = NA)
      dimnames(alt.test) <- list(grouping.names, names(lme4::fixef(model))[original.no.estex])
      for (i in 1:n.groups) {
        if (count == TRUE) {
          print(n.groups + 1 - i)
        }
        ...
        altered.no.estex <- which(substr(names(lme4::fixef(model.updated)), 
                                         1, 6) != "estex.")
    ...
    if (!is.null(select)) {
      ...
      altered.no.estex <- which(substr(names(lme4::fixef(model.updated)), 
                                       1, 6) != "estex.")
      alt.fixed <- matrix(ncol = n.pred, nrow = 1, data = lme4::fixef(model.updated)[altered.no.estex])
      dimnames(alt.fixed) <- list("Altered model", names(lme4::fixef(model.updated))[altered.no.estex])
      alt.se <- matrix(ncol = n.pred, nrow = 1, data = se.fixef(model.updated)[altered.no.estex])
      dimnames(alt.se) <- list("Altered model", names(lme4::fixef(model.updated))[altered.no.estex])
      alt.vcov <- list()
      ...
      alt.test <- matrix(ncol = n.pred, nrow = 1, data = coef(summary(model.updated))[, 
                                                                                      3][altered.no.estex])
      dimnames(alt.test) <- list("Altered model", names(lme4::fixef(model.updated))[altered.no.estex])
    }
  }
  if (obs) {
    if (is.null(select)) {
...
exclude.influenceJB <- function (model, grouping = NULL, level = NULL, obs = NULL, gf = "single", 
          delete = TRUE) 
{
  data.adapted <- model.frame(model)
  ##### Change Made Here #####
  ...
  if (delete == TRUE) {
    ...
    for (i in 1:length(level)) {
      data.adapted <- subset(data.adapted, data.adapted[, 
                                                        group.var] != level[i])
    }
    model.updated <- update(model, data = data.adapted)
    return(model.updated)
  ...
  if (names(data.adapted)[2] != "intercept.alt") {
    data.adapted$intercept.alt <- ifelse(model@flist[, grouping] == 
                                           level[1], 0, 1)
    data.adapted[, ncol(data.adapted) + 1] <- ifelse(model@flist[, 
                                                                 grouping] == level[1], 1, 0)
    added.variables <- make.names(paste("estex.", as.character(level[1]), 
                                        sep = ""))
    colnames(data.adapted)[ncol(data.adapted)] <- added.variables
    if (length(level) > 1) {
      for (i in 2:length(level)) {
        data.adapted$intercept.alt[model@flist[, grouping] == 
                                     level[i]] <- 0
        data.adapted[, ncol(data.adapted) + 1] <- ifelse(model@flist[, 
                                                                     grouping] == level[i], 1, 0)
        added.variables <- append(added.variables, values = make.names(paste("estex.", 
                                                                             as.character(level[i]), sep = "")))
    ...
    if (gf == "single") {
      ...
      if (length(names(ranef(model)[[grnr]])) == 1) {
        model.updated <- update(model, formula = as.formula(paste(". ~ 0 + intercept.alt +", 
                                                                  paste(added.variables, collapse = "+"), "+ .", 
                                                                  "- (1 |", grouping, ") + (0 + intercept.alt |", 
                                                                  grouping, ")")), data = data.adapted)
      }
      if (length(names(ranef(model)[[grnr]])) > 1) {
        model.updated <- update(model, formula = as.formula(paste(". ~ 0 + intercept.alt + ", 
                                                                  paste(added.variables, collapse = "+"), " + .", 
                                                                  paste(" - (", paste(names(ranef(model)[[grnr]])[-1], 
                                                                                      collapse = "+"), "|", grouping, ")"), " + (0 + intercept.alt +", 
                                                                  paste(names(ranef(model)[[grnr]])[-1], collapse = "+"), 
                                                                  "|", grouping, ")")), data = data.adapted)
      }
    }
    if (gf == "all") {
      ...
      for (i in 1:length(ranef(model))) {
        if (length(names(ranef(model)[[i]])) > 1) {
          delete.gf[i] <- paste("- (", paste(names(ranef(model)[[i]][-1]), 
                                             collapse = "+"), "|", names(ranef(model))[i], 
                                ")")
        }
        if (length(names(ranef(model)[[i]])) == 1) {
